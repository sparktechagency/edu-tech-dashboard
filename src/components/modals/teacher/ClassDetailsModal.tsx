...
const ClassDetailsModal: React.FC<ClassScheduleDetailsModalProps> = ({ visible, onClose, classData }) => {
    if (!classData) return null;

    const details = [
        { label: 'Title', value: classData.title },
        { label: 'Description', value: classData.description },
        { label: 'Class Date', value: new Date(classData.date).toLocaleDateString() },
        { label: 'Time', value: new Date(classData.time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) },
        { label: 'Location', value: classData.location },
        { label: 'Target', value: 'Fullstack', isTag: true, tagColor: '#F3F4F6', textColor: '#374151' },
        { label: 'Status', value: classData.status, isTag: true, tagColor: '#E6F9F0', textColor: '#22C55E' },
    ];
    ...
    return (
        <Modal
            ...
            width={600}
            centered
            title={<span className="text-xl font-semibold text-gray-800">Class Schedule Details</span>}
